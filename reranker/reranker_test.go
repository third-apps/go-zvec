...
func TestRRFReranker(t *testing.T) {
	r1 := []flat.SearchResult{
		{PK: "doc_a", Score: 0.9},
		{PK: "doc_b", Score: 0.5},
		{PK: "doc_c", Score: 0.3},
	}
	r2 := []flat.SearchResult{
		{PK: "doc_b", Score: 0.8},
		{PK: "doc_d", Score: 0.7},
		{PK: "doc_a", Score: 0.2},
	}
	...
	results := params.Rerank([][]flat.SearchResult{r1, r2}, 5)
...
func TestWeightedReranker(t *testing.T) {
	...
	results := params.Rerank([][]flat.SearchResult{r1, r2}, 3)
...
func TestEmptyResults(t *testing.T) {
	params := NewRRFParams(60)
	results := params.Rerank([][]flat.SearchResult{{}}, 10)
	if len(results) != 0 {
...
func TestRerankWithInterface(t *testing.T) {
	r1 := []flat.SearchResult{{PK: "a", Score: 1.0}, {PK: "b", Score: 0.5}}
	r2 := []flat.SearchResult{{PK: "b", Score: 0.8}, {PK: "c", Score: 0.3}}

	results := Rerank(NewRRFParams(60), [][]flat.SearchResult{r1, r2}, 3)
	if len(results) != 3 {
		t.Fatalf("expected 3 results, got %d", len(results))
	}
}
